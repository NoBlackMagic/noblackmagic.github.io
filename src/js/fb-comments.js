...
(function(){
    ...
    if (options.thread) {
        ...
        if (options.url.indexOf('localhost') !== -1) {
            document.write(
                '<div style="background:#eee;padding: 2px 5px;text-align:center;font-style:italic">' +
                    'Facebook comments are disabled from <b>localhost</b>' +
                '</div>'
            );
            return;
        }
        ...
        document.write(
            '<div ' +
            'id="' + options.id + '" ' +
            'class="fb-comments ' + options.class + '" ' +
            'style="' + options.style + '" ' +
            'data-href="' + options.url + '" ' +
            'data-width="' + options.width + '" ' +
            'data-numposts="' + options.posts + '" ' +
